...
namespace Hazel {
	...
	{
		GLint loaction = glGetUniformLocation(m_RendererID, name.c_str());
		glUniform2f(loaction, value.x, value.y);
	}

	void OpenGLShader::UploadUniformFloat3(const std::string& name, const glm::vec3& value)
	{
		GLint loaction = glGetUniformLocation(m_RendererID, name.c_str());
		glUniform3f(loaction, value.x, value.y, value.z);
	}
	...
	void OpenGLShader::Compile(const std::unordered_map<GLenum, std::string>& shaderSources)
	{
		...
		{
			HZ_CORE_ERROR("No shader source provided!");
			return;
		}

		GLuint program = glCreateProgram();
		HZ_CORE_ASSERT(shaderSources.size() <= 2, "OpenGLShader currently supports only 2 shaders!");
		...
		for (auto& kv : shaderSources)
		{
			...
			const GLchar* sourceCSstr = (const GLchar*)source.c_str();
			glShaderSource(shader, 1, &sourceCSstr, 0);
